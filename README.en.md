...
Recovery update: before dequeueing, Codex account availability and rate limits are checked. Auth/quota failures preserve pending input with bounded backoff and jitter; notices have a five-minute cooldown. A ten-second watchdog reconciles only the exact tracked terminal turn after missed completion. Desktop owners are freshly discovered before each start. `/codex status` shows counts/reasons/retry time; `/codex retry` rechecks safe pending work only. Ambiguous deliveries are quarantined, never blindly resent. Inspect Codex before `/codex retract`; it cannot undo an executed turn. See [SETUP.md](SETUP.md). No logout, token reissue or quota-reset redemption is automated.
